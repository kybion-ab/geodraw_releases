...
Missing bindings:
  - geodraw.ShapeEditor class
    - register_commands(app)   — installs CTRL+S/R/L/… bindings
    - add_to_scene(scene)      — draws the in-progress and committed geometry
    - handle_input(app)        — processes mouse clicks / drags (draw callback)
  - scene.add_ground() with depth-buffer writes (opaque ground for CTRL+click picking)
  - scene.add_polyline() already exists for the grid lines
...
def main():
    raise NotImplementedError(
        "demo_shape_edit requires missing Python bindings: "
        "geodraw.ShapeEditor (register_commands, add_to_scene, handle_input)."
    )
